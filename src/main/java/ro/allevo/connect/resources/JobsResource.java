...
public class JobsResource extends QuartzPagedApiCollection<JobEntity>{
	...
	public JobsResource(EntityManager entityManagerConnect, EntityManager entityManagerList, ConnectionFactory connectionFactory, String amqStatement) {
	...
	public Response createJob(JobEntity jobRequest) {
		try {
			...
			Map<String,Object> customJobData  = new HashMap<String,Object>();
			switch(jobType) {
				case PSD2:
	...
	private void initPsd2Job(Map<String,Object> customJobData, JobEntity jobRequest) {
		TypedQuery<AuthorizationServersEntity> queryAuth = entityManagerConnect.createNamedQuery("AuthorizationServersEntity.findByBIC", AuthorizationServersEntity.class).setParameter("bic", jobRequest.getName());
		AuthorizationServersEntity auth = queryAuth.getSingleResult();	
		TypedQuery<InternalAccountEntity> internalAccount = entityManagerList.createNamedQuery("InternalAccountEntity.findByBIC", InternalAccountEntity.class).setParameter("bic", jobRequest.getName());
