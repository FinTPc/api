...
public class PagedCollection<T extends BaseEntity> implements AutoCloseable, Serializable {
	...
	private void getPage() {
		...
		if (entityManager != null && entityClass != null) {
			JpaFilter<T> jpaFilter = new JpaFilter<T>(uriInfo, entityManager, entityClass);
	...
	protected Response post(BaseEntity entity, boolean fetchNewID) {
		...
		try {		
			// perform update
			EntityManagerUtils.persistEntityToDB(entity, entityManager, uriInfo);
			...
			logger.error("Could not parse time when creating statusEntity in ModelUtils");
		} 		
	...
	protected Response patch(BaseEntity entity, boolean fetchNewID) {
		if (null == entityManager)
			throw new UnsupportedOperationException(ERROR_UNSUPPORTED_OPERATION_ENTITY_EXCEPTION);

		try {
			// patch update
			EntityManager eml = entityManager.getEntityManagerFactory().createEntityManager();
	...
	protected Response patch(T entity) {
		return patch(entity, true);
	}

	protected void rollback(T entity) {
		if (null == entityManager)
			throw new UnsupportedOperationException(ERROR_UNSUPPORTED_OPERATION_ENTITY_EXCEPTION);
