...
public class MessagesResource extends PagedCollection<RoutedMessageEntity> {
	...
	public MessageResource getMessageByCorrelationId(@PathParam("id") String correlationId) {
		...
		return MessageResource.getByCorrelationId(getUriInfo(), getEntityManager(), correlationId);
	}
	...
	public MessagesByMessageTypeResource getMessagesByMessageType(@PathParam("messageType") String messageTypeValue) {
		...
		MessageTypeToViews messageType = MessageTypeToViews.get(
				getEntityManager(),
				messageTypeValue
				);
	...
	@JsonIgnore
	public PagedCollection<RoutedMessageEntity> getMessagesAsJson(@Context HttpServletRequest request) {
		setRequest(request);
		return this;
	}
