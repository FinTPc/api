...
public class UserResource extends BaseResource<UserEntity> {
	...
	public List<UserDefinedRoleEntity> getAuthorizedRoles(String messageType, String entity, UserRoleEntity.Action action) {
		List<UserDefinedRoleEntity> authorized = new ArrayList<UserDefinedRoleEntity>();
		
		for (UserRoleEntity role : getRoles())
			if (role.getAction().equals(action.toString()))
				for (UserDefinedRoleEntity udRole : role.getRoleEntity().getUserDefinedRoles())
					if (
						(messageType == null || udRole.getMessageType().equals(messageType)) //null skips check
	...
	public Set<String> getAuthorizedEntities(String messageType, UserRoleEntity.Action action) {
		Set<String> entities = new HashSet<String>();
		
		entities.add(""); // handle not authorized for select ... where x in ('', ...)
		
		List<UserDefinedRoleEntity> roles = getAuthorizedRoles(messageType, null, action);
