				...
				switch (filterType) {
					case FILTER_TYPE_EXACT:
						predicate = cb.and(predicate,
								path.get(fieldName).in(filter.getValues()));
						break;
					case FILTER_TYPE_NOTEXACT:
						predicate = cb.and(predicate,
								cb.notEqual(path.get(fieldName), fieldValue));
						break;
					case FILTER_TYPE_CONTAINS:
						predicate = cb.and(predicate, cb.like(
								cb.upper(path.<String> get(fieldName)), "%"
										+ fieldValue.toUpperCase() + "%"));
						break;
					case FILTER_TYPE_EXACTOR:
						predicate = cb.or(predicate,
								cb.equal(path.get(fieldName), fieldValue));
						break;
					...
					case FILTER_TYPE_LDATE:							
						predicate = cb.and(predicate, cb.lessThanOrEqualTo(path.<Date> get(fieldName),  sdf.parse(fieldValue)));
//						if(fieldName.equalsIgnoreCase("eventdate")) {
//							break;
//						}
						break;
					case FILTER_TYPE_UDATE:											
						predicate = cb.and(predicate, cb.greaterThanOrEqualTo(path.<Date> get(fieldName), sdf.parse(fieldValue)));
						break;
					case FILTER_TYPE_END:
						predicate = cb.and(predicate, cb.lessThanOrEqualTo(
								path.<Timestamp> get(fieldName),
								ResourcesUtils.getTimestamp(fieldValue)));
						break;
					default:
						switch (FilterJavaType.fromName(path.get(fieldName)
								.getJavaType().toString())) {
								case FILTER_TYPE_BIGDECIMAL:
									predicate = cb.and(predicate, cb.equal(
											path.<BigDecimal> get(fieldName),
											fieldValue));
									break;
...
enum FilterJavaType {
	FILTER_TYPE_TIMESTAMP("java.sql.Timestamp"), 
	FILTER_TYPE_STRING("java.lang.String"), 
